classdef FLW_compute_ICA_merged<CLW_generic
    properties
    ...
    methods
        ...
        function obj = FLW_compute_ICA_merged(batch_handle)
            %call the constructor of the superclass
            %CLW_generic(tabgp,fun_name,suffix_name,help_str)
            obj@CLW_generic(batch_handle,'ICA_merged','ica_merged',...
                ['This function is identical to the COMPUTE ICA MATRIX function ',...
                'except for the fact that ICA is performed after merging ',...
                'multiple datasets. The obtained ICA matrix is then ',...
    ...
    methods (Static = true)
        function lwdataset_out= get_headerset(lwdataset_in,option)
            ...
            lwdataset_out=[];
            for k=1:N
                lwdataset_out(end+1).header=FLW_compute_ICA_merged.get_header(lwdataset_in(k).header,option);
        ...
        function lwdataset_out= get_lwdataset(lwdataset_in,varargin)
            ...
            for k=1:N
                lwdataset_out(k).header.history(end).option.unmix_matrix=ica_um;
                lwdataset_out(k).header.history(end).option.mix_matrix=pinv(ica_um);

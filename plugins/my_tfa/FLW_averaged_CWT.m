classdef FLW_averaged_CWT<CLW_generic
    properties
        FLW_TYPE=1;
    ...
    methods
        function obj = FLW_averaged_CWT(batch_handle)
            obj@CLW_generic(batch_handle,'avg_CWT','avg cwt',...
                ['Compute the averaging results of the Continuous Wavelet Transform.',...
                ' This operation equals to the operation cwt and avg, but save the memory greatly.']);
    ...
    methods (Static = true)
        ...
        function lwdata_out=get_lwdata(lwdata_in,varargin)
            ...
            header=FLW_averaged_CWT.get_header(lwdata_in.header,option);
            ...
            for epochpos=1:lwdata_in.header.datasize(1)
                for channelpos=1:header.datasize(2)
                    for indexpos=1:header.datasize(3)
                        temp=cwt(lwdata_in.data(epochpos,channelpos,indexpos,1,1,:),scales,option.wavelet_name);
                        switch option.output
                            case 'amplitude'
                                temp=abs(temp);
                            case 'power'
                        ...
                        data(1,channelpos,indexpos,1,:,:)=squeeze(data(1,channelpos,indexpos,1,:,:))+temp;
                        t=toc;
                        if option.show_progress==1 && ishandle(fig) && t-t1>0.2
                            t1=t;
                            N=(((epochpos-1)*header.datasize(2)+channelpos-1)...
                                *header.datasize(3)+indexpos)/...
                                (header.datasize(3)*header.datasize(2)*lwdata_in.header.datasize(1));
